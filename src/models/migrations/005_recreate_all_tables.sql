-- Миграция 005: пересоздание всех таблиц (DROP и CREATE) с добавлением колонки training_type (FALSE = individual, TRUE = group) в таблицу training_sessions
DROP TABLE IF EXISTS training_sessions CASCADE;
DROP TABLE IF EXISTS session_participants CASCADE;
DROP TABLE IF EXISTS individual_training_sessions CASCADE;
...
-- Таблица администраторов (administrators)
...
-- Таблица тренировок (training_sessions) с добавленной колонкой training_type (FALSE = individual, TRUE = group)
...
-- Таблица участников тренировок (session_participants)
...
-- Таблица индивидуальных тренировок (individual_training_sessions)
CREATE TABLE individual_training_sessions (
    id SERIAL PRIMARY KEY,
    client_id INTEGER REFERENCES clients(id) ON DELETE CASCADE,
    child_id INTEGER REFERENCES children(id),
    equipment_type VARCHAR(20) NOT NULL, -- ski, snowboard
...
-- Таблица заявок на тренировки (training_requests)
CREATE TABLE training_requests (
    ...
    group_size INTEGER,
    training_frequency VARCHAR(20), -- regular, one-time
    skill_level INTEGER CHECK (skill_level BETWEEN 0 AND 10),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- Таблица сертификатов (certificates)
...
-- Таблица неудачных платежей (failed_payments)
...
-- Создание индексов (индексы для training_sessions, session_participants, clients, children, trainers, administrators, certificates, wallets, transactions, schedule, training_requests, individual_training_sessions, failed_payments)
...
-- Создание триггеров для обновления updated_at (update_updated_at_column) на всех таблицах (clients, children, trainers, administrators, training_sessions, session_participants, certificates, schedule, training_requests, individual_training_sessions, failed_payments)
...
CREATE TRIGGER update_individual_training_updated_at BEFORE UPDATE ON individual_training_sessions FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
-- Миграция 005: пересоздание всех таблиц (DROP и CREATE) с добавлением колонки training_type (FALSE = individual, TRUE = group) в таблицу training_sessions (для истории). 