/**
 * Скрипт для создания начальной записи политики конфиденциальности
 * 
 * Использование:
 * node src/scripts/init-privacy-policy.js
 * 
 * Подключение к БД через переменные окружения из .env
 */

require('dotenv').config();
const { Pool } = require('pg');

// Подключение к БД
const pool = new Pool({
    host: process.env.DB_HOST,
    port: process.env.DB_PORT,
    database: process.env.DB_NAME,
    user: process.env.DB_USER,
    password: process.env.DB_PASSWORD,
    ssl: false
});

/**
 * Создает начальную запись политики конфиденциальности
 */
async function initPrivacyPolicy() {
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
        
        // Проверяем, есть ли уже активная политика
        const existingResult = await client.query(
            `SELECT id, version FROM privacy_policies 
             WHERE is_active = true 
             ORDER BY effective_date DESC 
             LIMIT 1`
        );
        
        if (existingResult.rows.length > 0) {
            console.log(`✅ Активная политика уже существует: версия ${existingResult.rows[0].version}, ID: ${existingResult.rows[0].id}`);
            await client.query('COMMIT');
            return existingResult.rows[0];
        }
        
        // Создаем начальную политику
        // Содержимое можно взять из views/privacy-policy.ejs или оставить краткое описание
        const policyContent = `
Горностайл72 - Политика конфиденциальности

Дата последнего обновления: 11 сентября 2025 года

Общие положения
Настоящее согласие на обработку персональных данных (далее — «Согласие») дается в соответствии с требованиями Федерального закона от 27.07.2006 № 152-ФЗ «О персональных данных».

Ответственный за персональные данные:
- Самозанятый: Тебякин Данила Юрьевич
- ИНН: 740416017714
- Паспорт: 7115 190305
- Деятельность: Тренер, инструктор и др.
- Адрес: 625048, Тюменская область, г. Тюмень, с. Яр, ул. Источник, 2А
- Телефон: +7 (912) 392-49-56
- Email: batl-zlat@yandex.ru

Цели обработки персональных данных:
1. Регистрация клиентов в системе управления тренировками
2. Организация тренировочного процесса (запись на тренировки, составление расписания)
3. Финансовые операции (ведение кошельков, обработка платежей, выдача сертификатов)
4. Коммуникация с клиентами (уведомления, напоминания, ответы на запросы)
5. Ведение статистики и аналитики

Подробная информация доступна на странице: https://gornostyle72.ru/privacy-policy
        `.trim();
        
        const result = await client.query(
            `INSERT INTO privacy_policies (version, title, content, is_active, effective_date)
             VALUES ($1, $2, $3, $4, $5)
             RETURNING id, version`,
            [
                '1.0',
                'Политика конфиденциальности и обработки персональных данных',
                policyContent,
                true,
                new Date('2025-09-11')
            ]
        );
        
        await client.query('COMMIT');
        
        console.log(`✅ Создана начальная политика конфиденциальности:`);
        console.log(`   Версия: ${result.rows[0].version}`);
        console.log(`   ID: ${result.rows[0].id}`);
        console.log(`   Дата вступления в силу: 2025-09-11`);
        
        return result.rows[0];
    } catch (error) {
        await client.query('ROLLBACK');
        throw error;
    } finally {
        client.release();
    }
}

/**
 * Главная функция
 */
async function main() {
    try {
        console.log('🚀 Инициализация политики конфиденциальности...\n');
        
        const policy = await initPrivacyPolicy();
        
        console.log('\n✅ Инициализация завершена успешно!');
        process.exit(0);
    } catch (error) {
        console.error('❌ Ошибка при инициализации:', error);
        process.exit(1);
    } finally {
        await pool.end();
    }
}

// Запуск скрипта
if (require.main === module) {
    main();
}

module.exports = { initPrivacyPolicy };

