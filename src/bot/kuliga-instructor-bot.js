...
try {
    ...
    console.log('🤖 Бот инструкторов Кулиги и Воронинских горок запущен...');
} catch (error) {
...
bot.onText(/\/start(?: (.+))?/, async (msg, match) => {
    ...
    try {
        ...
        if (instructorRes.rows.length === 0) {
            return bot.sendMessage(chatId,
                '❌ Вы не зарегистрированы как инструктор.\n\n' +
        ...
        await bot.sendMessage(chatId,
            ...
            'Это информационный бот для инструкторов Кулиги и Воронинских горок.\n\n' +
            'Здесь вы можете:\n\n' +
