...
## Уроки
(Будет заполняться в процессе выполнения) 

# План управления версиями и резервным копированием

## Background and Motivation
- Проект готов к первому релизу на удаленный сервер
- Необходимо обеспечить надежное управление версиями
- Требуется настройка автоматического резервного копирования базы данных

## Key Challenges and Analysis
1. Управление версиями:
   - Необходимость сохранения стабильной версии для возможности отката
   - Организация ветвления для разработки новых функций
   - Правильное тегирование релизов
...
- [ ] Создать тег для текущей стабильной версии (v1.0.0)
- [ ] Настроить .gitignore для исключения ненужных файлов
- [ ] Создать ветку main/master для стабильных версий
- [ ] Создать ветку develop для разработки
- [ ] Настроить защиту ветки main/master от прямых коммитов

### 2. Настройка резервного копирования БД (выполнено)
- [x] Определить тип используемой БД (PostgreSQL)
- [x] Создать скрипт (backup_database.sh) для создания бэкапа (хранится в /root/scripts/backup_database.sh)
- [x] Настроить cron (запуск ежедневно в 2:00) для автоматического бэкапа
- [x] Настроить ротацию (удаление бэкапов старше 30 дней) и логирование (в /root/gornostyle-backups/backup.log)
- [x] Документировать процедуру восстановления (см. README.md, раздел «Как восстановить базу данных из бэкапа»)

### 3. Документация и процедуры
- [ ] Создать документацию по процессу релиза
- [ ] Описать процедуру отката к предыдущей версии
- [ ] Документировать процесс восстановления из бэкапа
- [ ] Создать чек-лист для проверки перед релизом
...
## Рекомендации по хранению бэкапов (текущая реализация)
1. Локальное хранение (на сервере):
    – Бэкапы хранятся в директории /root/gornostyle-backups (с автоматической ротацией и логами в /root/gornostyle-backups/backup.log)
    – (Примечание: синхронизация с облаком не реализована.)
2. Периодичность: ежедневно (запуск в 2:00 ночи) с автоматическим удалением бэкапов старше 30 дней.

## Процедура восстановления
1. Из системы контроля версий (например, git checkout <tag> или git reset ––hard <commit>)
2. Из бэкапа базы данных (на сервере):
    – Подключитесь по SSH (ssh –i ~/.ssh/gornostyle_backup root@90.156.210.24)
    – Перейдите в папку с бэкапами (cd /root/gornostyle-backups)
    – Выберите нужный файл (например, skisimulator_2025-06-16_17-48-18.sql.gz)
    – Выполните команду (см. README.md, раздел «Как восстановить базу данных из бэкапа»):
      ...
      unset PGPASSWORD
    – (Примечание: перед восстановлением убедитесь, что приложение остановлено.) 